...
or Perl's <a
href="http://www.perl.com/doc/manual/html/pod/perldebug.html">perl5db</a>
debugger.
...
or Perl's perl5db debugger - \f(CWperldebug(1)\fR.
...
debugged, add C<--> before the script name. That will tell bashdb not
...
The long options listed befow are available if the C<getopts> command
is installed on your system. The short options can always be used.
...
Similar to C<set -x> line tracing except the I<location> of each line,
the bash level, and subshell level are printed; the indentation of
the original program is also preserved in the source output.

Z<>

=item -Y | --vtrace

C<-X> tracing except the the command is printed with string and
backtick expansions performed. So this is roughly like C<set -xv> but
hopefully more helpful. However note that because backtick expansion
is performed, it is possible that your program can behave differently
if those backticks cause state to change in a way that has an effect
on the program.
...
version 2.05 bash or earlier versions. In versions after 3.0 should
have been enabled when bash was built. (I think this is usually the
case though.) If you try to run the bashdb script on such as shell,
may get the message:
...
I<$Id: bashdb-man.pod,v 1.4 2006/12/28 12:34:25 rockyb Exp $>
